function addStateFromFormToDFAStatesList(form) { 
    ...
    if($("#DFAFinalStateCheckBox").is(':checked'))  { 
        ...
        //adding state to the final states array for graph 
construction 
...
$("#dfaTransitions").append("State: " + stateName + " |  Transitions: " + DFATransitionsRawInput + "<br/>"); 
    ...
    if(!FLAG_isStartStateExists && isCurrentStateStartingStateFlag == 
1)    
{ 
...
''); 
    ...
    if(DFATransitionsRawInput != ""){ 
        DFATransitionsStringToken = 
DFATransitionsRawInput.split(','); 
...
transitions) { 
    ...
    if (transitions.length <= 0) 
        ...
        for (var stateSymbol in transitions) { 
            //get the next state from array of stateSymbols 
individually  
            var nxtStateForSymbol = transitions[stateSymbol]; 
            // find obj in dfaStatesListArray 
            var nxtDFAStateObject = 
getStateFromStatesList(nxtStateForSymbol); 
            console.log(nxtDFAStateObject); 
if(nxtDFAStateObject != null) { 
                currentStateObject.next[stateSymbol] = 
nxtDFAStateObject; 
                //don't span state if it has loop else it will go for 
infinite 
                if(nxtDFAStateObject.isCurrentStateHasLoop != 1)    { 
                    currentStateObject.isCurrentStateHasLoop = 1; 
                    //again loop recursively until base case is 
reached 
